...
class Actions_controller extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        else
        {
            ...
            $this->breadcrumb                                                   =   $this->_actions_model->get_breadcrumb('ACTIONS');

            if($this->session->userdata['id_role'] != "11")
            {
                if(in_array('ACTIONS', $this->session->userdata['modules']) == FALSE)
                {
    ...
    **/
    public function view()
    {
        ...
        if ($this->actions != FALSE)
        {
            ...
            $this->_view->assign('path_view',                                   site_url('actions/datatable'));
            $this->_view->assign('path_add',                                    site_url('actions/add'));
            $this->_view->assign('path_trace',                                  site_url('actions/trace'));
            $this->_view->assign('path_export_xlsx',                            site_url('actions/exportxlsx'));

            $this->_view->display('admin/actions.tpl');
        }
        else
        {
            header("Location: " . $this->session->userdata['initial_site']);
    ...
    **/
    public function datatable()
    {
        if(in_array('VIEW', $this->actions))
        {
            ...
            $count_rows                                                         =   $this->_actions_model->count_rows($search);
            $all_rows                                                           =   $this->_actions_model->all_rows($limit, $start, $search, $order, $dir);
    ...
    public function add()
    {
        if(in_array('ADD', $this->actions))
        {
            ...
            if ($params)
            {
                ...
                if ($exist_action['data'])
                {
                    $add                                                        =   $this->_actions_model->add($params);
    ...
    public function edit()
    {
        if(in_array('EDIT', $this->actions))
        {
            ...
            if ($params)
            {
                ...
                if ($exist_action['data'])
                {
                    $edit                                                       =   $this->_actions_model->edit($params);
    ...
    **/
    public function trace()
    {
        if(in_array('TRACE', $this->actions))
        {
            ...
            if ($params)
            {
                $trace                                                          =   $this->_actions_model->trace_register($params);
    ...
    **/
    public function export_xlsx()
    {
        if(in_array('EXPORTXLSX', $this->actions))
        {
            ...
            $export_xlsx                                                        =   $this->_actions_model->export_xlsx($search);
            $count                                                              =   2;
            ...
            $sheet->setTitle('Acciones');

            header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            header('Content-Disposition: attachment;filename="trabajandofet_acciones_' . date('dmY') . '.xlsx"');
